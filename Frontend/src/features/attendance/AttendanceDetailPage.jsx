  ...
  return <main className="app-shell">
    ...
    <form className="form-card attendance-detail-card" onSubmit={save}>
      <div className="detail-grid">
        ...
        <div className="detail-field"><span>Worked Hours</span><strong>{record.workedHours == null ? "-" : record.workedHours.toFixed(2)}</strong></div><div className="detail-field"><span>Overtime</span><strong>{record.scheduleApplied ? (record.overtime == null ? "-" : `${record.overtime.toFixed(2)} hrs`) : "-"}</strong><small>{record.scheduleApplied ? `Measured against: ${record.scheduleName || "assigned schedule"}` : "No schedule assigned — overtime not calculated"}</small></div>
